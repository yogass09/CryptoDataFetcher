...
crypto_df <- crypto_df %>%
  group_by(ticker) %>%
  mutate(
    downside_returns = ifelse(`1d%` < 0, `1d%`, NA),  # Extract downside returns
    downside_deviation = sd(downside_returns, na.rm = TRUE),  # Calculate downside deviation
    sortino_ratio = (mean(`1d%`, na.rm = TRUE) - rf) / downside_deviation  # Calculate Sortino Ratio
  ) %>%
  ungroup()
...
sortino_ratio_df <- crypto_df %>%
  group_by(ticker) %>%
  summarise(
    downside_returns = mean(ifelse(`1d%` < 0, `1d%`, NA), na.rm = TRUE),  # Average of downside returns
    average_return = mean(`1d%`, na.rm = TRUE),  # Average of all daily returns
    downside_deviation = sd(ifelse(`1d%` < 0, `1d%`, NA), na.rm = TRUE),  # Standard deviation of downside returns
